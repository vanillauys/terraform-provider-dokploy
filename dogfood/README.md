...
| `generate_imports.py` | Write Terraform `import` blocks for the live resources that this provider supports, **except** the data volumes of the database engines (see below), the vault providers, the users, and a compose service without a usable source. The script lists each vault provider as a comment: the server redacts the config block, so you import it by hand and supply the block for its type. It lists each member as a comment too, because Dokploy never returns a password, and imports the member's permissions instead. It skips, with a `# skipped` comment, a compose service whose source is not configured yet or whose source type this provider does not model: `dokploy_compose` requires exactly one source block, so such a record has no valid configuration. |
