...
- `database_password` (String, Sensitive) PostgreSQL password. A change starts a redeploy. Set this attribute or `database_password_wo`.
...
- `docker_image` (String) Postgres Docker image, for example `postgres:16-alpine`. If you omit it, the server default applies.
...
- `external_port` (Number) Host port for PostgreSQL. If unset, the database stays internal.
