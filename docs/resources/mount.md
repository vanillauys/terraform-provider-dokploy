...
- `service_type` (String) Kind of service `service_id` refers to: one of `application`, `postgres`, `mysql`, `mariadb`, `mongo`, `redis`, `compose`, `libsql`. Changing it forces replacement, for the same reason as `service_id`. `compose` and `libsql` are accepted by the server but have no corresponding resource in this provider yet.
