...
func TestCreateAndGetMariadb(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodPost && r.URL.Path == "/api/mariadb.create":
			raw, _ := io.ReadAll(r.Body)
			_ = json.Unmarshal(raw, &createBody)
			_, _ = fmt.Fprint(w, mariadbJSON)
		case r.Method == http.MethodGet && r.URL.Path == "/api/mariadb.one":
			if r.URL.Query().Get("mariadbId") != "md1" {
	...
	c := testClient(t, srv)
	md, err := c.CreateMariadb(context.Background(), CreateMariadbRequest{
		Name: "db", DatabaseName: "app", DatabaseUser: "app",
		DatabasePassword: "hunter2", EnvironmentID: "e1",
	})
	if err != nil {
		t.Fatalf("CreateMariadb: %v", err)
	}
	if md.MariadbID != "md1" {
		t.Errorf("mariadb = %+v", md)
	}
	...
	got, err := c.GetMariadb(context.Background(), "md1")
	if err != nil {
		t.Fatalf("GetMariadb: %v", err)
	}
	...
	if got.Name != "db" || got.AppName != "db-x1y2" || got.DatabaseName != "app" ||
		...
		got.DatabaseRootPassword != "r00t" || got.DockerImage != "mariadb:11.4" ||
		got.ExternalPort == nil || *got.ExternalPort != 3307 ||
...
// TestCreateMariadbOmitsEmptyRootPassword pins the same omit-vs-empty
// resolution as mysql (internal/client/doc.go: "same databaseRootPassword
// behavior as mysql"): a zero-value (unset) DatabaseRootPassword must vanish
// from the request body entirely, not marshal as "databaseRootPassword":"".
// Verified live (2026-07-27) that mariadb.create then generates a random
// password, matching an entirely absent key.
func TestCreateMariadbOmitsEmptyRootPassword(t *testing.T) {
	var createBody map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost || r.URL.Path != "/api/mariadb.create" {
			t.Errorf("unexpected request: %s %s", r.Method, r.URL.Path)
		}
		raw, _ := io.ReadAll(r.Body)
		_ = json.Unmarshal(raw, &createBody)
	...
	c := testClient(t, srv)
	if _, err := c.CreateMariadb(context.Background(), CreateMariadbRequest{
		Name: "db", DatabaseName: "app", DatabaseUser: "app",
		DatabasePassword: "hunter2", EnvironmentID: "e1",
...
// TestCreateMariadbSendsExplicitRootPassword pins the other half: a
// caller-supplied value must reach the server verbatim.
func TestCreateMariadbSendsExplicitRootPassword(t *testing.T) {
	var createBody map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost || r.URL.Path != "/api/mariadb.create" {
			t.Errorf("unexpected request: %s %s", r.Method, r.URL.Path)
...
func TestMariadbMutations(t *testing.T) {
	var calls []string
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/mariadb.update":
			...
			// dialect-B endpoint, same as mysql - the key must always be
			// present too, carrying whatever value the caller set (never
			// omitted, per UpdateMariadbRequest's doc comment).
			if _, ok := body["databaseRootPassword"]; !ok {
				t.Errorf("mariadb.update body missing required key %q: %v", "databaseRootPassword", body)
			}
	...
	c := testClient(t, srv)
	if err := c.UpdateMariadb(ctx, UpdateMariadbRequest{MariadbID: "md1", Name: "renamed", DatabaseRootPassword: ""}); err != nil {
		t.Fatal(err)
	}
	if err := c.UpdateMariadb(ctx, UpdateMariadbRequest{MariadbID: "md1", Name: "renamed", DatabaseRootPassword: "newroot"}); err != nil {
		t.Fatal(err)
	}
	env := "KEY=value"
	if err := c.SaveMariadbEnvironment(ctx, "md1", &env); err != nil {
		t.Fatal(err)
	}
	if err := c.SaveMariadbEnvironment(ctx, "md1", nil); err != nil {
		t.Fatal(err)
	}
	port := int64(3308)
	if err := c.SaveMariadbExternalPort(ctx, "md1", &port); err != nil {
		t.Fatal(err)
	}
	if err := c.SaveMariadbExternalPort(ctx, "md1", nil); err != nil {
		t.Fatal(err)
	}
	if err := c.DeployMariadb(ctx, "md1"); err != nil {
		t.Fatal(err)
	}
	if err := c.DeleteMariadb(ctx, "md1"); err != nil {
		t.Fatal(err)
	}
	want := []string{
		"POST /api/mariadb.update",
		"POST /api/mariadb.update",
		"POST /api/mariadb.saveEnvironment",
		"POST /api/mariadb.saveEnvironment",
		"POST /api/mariadb.saveExternalPort",
		"POST /api/mariadb.saveExternalPort",
	...
	if len(externalPortBodies) != 2 || externalPortBodies[0] != float64(3308) || externalPortBodies[1] != nil {
