...
func (d *libsqlDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"cpu_limit":           schema.StringAttribute{Computed: true, Description: "Hard CPU limit in nano-CPUs, for example `\"500000000\"` for half a CPU."},
			"cpu_reservation":     schema.StringAttribute{Computed: true, Description: "Reserved CPU in nano-CPUs, for example `\"250000000\"` for a quarter CPU."},
			"memory_limit":        schema.StringAttribute{Computed: true, Description: "Hard memory limit in bytes, for example `\"536870912\"` for 512 MiB."},
			"memory_reservation":  schema.StringAttribute{Computed: true, Description: "Reserved memory in bytes, for example `\"268435456\"` for 256 MiB."},
